...
// 발표용 아키텍처 실시간 트래킹 페이지 (다크 모드, 고대비).
// 데모(같은 출처 localhost:3000)의 단계/버튼이 바뀌면 archChannel 로 메시지가 와서
// 해당 노드가 발광(.active)하고 나머지는 흐려지며, 노드가 속한 탭으로 자동 전환된다.
//  · 탭 1: 분석단 · 진단 흐름 (세로)
//  · 탭 2: 솔루션단 · 실시간 경고 (세로)
...
export default function ArchitecturePage() {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: BG, color: TEXT, fontFamily: "Pretendard, sans-serif", display: "flex", flexDirection: "column" }}>
      <style>{`
      ...
      {/* 상단 바 */}
      <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 14, padding: "12px 22px", borderBottom: "1px solid #202D44", background: PANEL }}>
        <span style={{ fontSize: 12.5, fontWeight: 800, letterSpacing: 1, color: ACCENT, display: "inline-flex", alignItems: "center", gap: 6 }}>
          <span style={{ width: 8, height: 8, borderRadius: "50%", background: ACCENT, animation: "archdot 1.4s ease-in-out infinite" }} />LIVE
        </span>
        <span style={{ fontSize: 15, fontWeight: 700, color: TEXT }}>소프트웨어 아키텍처 · 실시간 추적</span>
        <div style={{ display: "flex", gap: 6, marginLeft: 10 }}>
          {([["analysis", "분석단 · 진단"], ["solution", "솔루션단 · 실시간"]] as const).map(([id, label]) => (
            <button key={id} onClick={() => { autoTab.current = false; setTab(id); }}
              style={{ cursor: "pointer", fontSize: 13, fontWeight: tab === id ? 700 : 500, borderRadius: 9, padding: "7px 14px", border: "1px solid " + (tab === id ? ACCENT : "#2C3A56"), background: tab === id ? "rgba(" + GLOW + ",.16)" : "transparent", color: tab === id ? "#fff" : "#9DB2C9" }}>
              {label}
            </button>
          ))}
        </div>
        <span style={{ marginLeft: "auto", fontSize: 14, fontWeight: 600, color: any ? ACCENT : "#8A95A6", maxWidth: "44vw", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>▶ {note}</span>
      </div>

      {/* 본문 */}
      <div className="arch-scroll" style={{ flex: 1, overflowY: "auto", padding: "30px 24px 90px" }}>
